...
function afficherCommandeC ($Commande){
		...
		echo "quArt: ".$Commande->getquArt()."<br>";
	...
	function ajouterCommande($Commande){
		$sql="insert into commande (idCom,idClient,quArt,prTotal,etatCom) values (:idCom,:idClient,:quArt,:prTotal,:etatCom)";
	...
	function modifierCommande($Commande,$idCom){
		$sql="UPDATE commande SET idCom=:idCom,idClient=:idClient,quArt=:quArt,prtotal=:prtotal,etatCom=:etatCom WHERE idCom=:idCom";
...
try{		
		...
		$quArt=$Commande->getquArt();
        $prTotal=$Commande->getprtotal();
        $etatCom=$Commande->getetatCom();

   
		$datas = array(':idCom'=>$idCom,':idClient'=>$idClient,':quArt'=>$quArt, ':prTotal'=>$prTotal, ':etatCom'=>$etatCom);
		$req->bindValue(':idCom',$idCom);
		$req->bindValue(':idClient',$idClient);
		$req->bindValue(':quArt',$quArt);
		$req->bindValue(':prTotal',$prTotal);
